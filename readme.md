...
list. For a longer quest, **Shellfall** is a bombardment courtyard (duck the turret behind cover
pillars, but keep moving — the spitter's shells arc right over them),
**The Warden's Locks** is a pressure-plate puzzle vault (weigh down a latch
plate to open the first lock, press both plates to trip a counter on the
second), **Shiftworks** is a shifting-floor traversal gauntlet (ride the conveyors,
sprint the crumbling tiles before they drop, time the sweeping blade, then
bounce to the star), **The Warded Hall** is a priority-target fight (a healer drone keeps the
