...
var CharacterController = (function () {
    function CharacterController(avatar, camera, scene, actionMap, faceForward) {
        ...
        this._jumpTime = 0;
        // Multi-jump + glide (custom extension). _maxJumps 1 = classic single
        // jump; 2 enables a mid-air double jump. Holding the jump key while
        // falling glides (slow constant descent) when _glideEnabled.
        this._maxJumps = 1;
        this._jumpsUsed = 0;
        this._glideEnabled = false;
        this._glideFallSpeed = 1.6;
    ...
    CharacterController.prototype._calcJumpDist = function (speed, dt) {
        ...
        // Gliding: while the jump key is held on the way DOWN, descend at a
        // slow constant speed instead of accelerating under gravity.
        if (this._glideEnabled && this._jumpKeyHeld && js < 0) {
            return -this._glideFallSpeed * dt;
        }
    ...
    CharacterController.prototype._endJump = function () {
        ...
        this._jumpTime = 0;
        this._jumpsUsed = 0;    // touching down restores all air jumps
        this._wasWalking = false;
        this._wasRunning = false;
    };
    // ---- multi-jump / glide public API (custom extension) ----
    // How many jumps in total before touching down (1 = classic, 2 = double jump).
    CharacterController.prototype.setJumpCount = function (n) {
        this._maxJumps = Math.max(1, n | 0);
    };
    // Holding the jump key while falling descends at a slow constant speed.
    CharacterController.prototype.enableGlide = function (b) {
        this._glideEnabled = !!b;
    };
    CharacterController.prototype.setGlideFallSpeed = function (n) {
        this._glideFallSpeed = Math.max(0.2, n);
    };
    ...
    // Added method to check if the avatar has ground directly below them, used to trigger
    // a more accurate freefall transition in _doMove() that should prevent a bug where
    // the player can walk across empty space simply by holding down the movement keys.
    CharacterController.prototype._checkGroundBelow = function() {
        this._rayLength = 5;

        // Define the starting point of the ray (the avatar's position)
        var rayStart = this._avatar.position;

        // Define the end point of the ray (directly below the avatar)
        var rayEnd = new BABYLON.Vector3(rayStart.x, rayStart.y - this._rayLength, rayStart.z);

        // Create the ray
        var ray = new BABYLON.Ray(rayStart, rayEnd.subtract(rayStart).normalize(), this._rayLength);
        ...
        // Check if the ray hit a solid object within the specified distance
        if (hit.hit && hit.pickedPoint && hit.distance <= this._rayLength) {
            return true; // Ground is below
        } else {
            return false; // Free fall
        }
    };

    // CharacterController.prototype._doMove is a method responsible for handling the movement of a character in a 3D environment.
    CharacterController.prototype._doMove = function (dt) {
        // Calculate the initial downward velocity due to gravity.
        var u = this._movFallTime * this._gravity;

        // Compute the free fall distance since the last frame, assuming the character is in free fall.
        this._freeFallDist = u * dt + this._gravity * dt * dt / 2;

        // Gliding: while the jump key is held during a free fall, descend at a
        // slow constant speed and don't keep accelerating.
        if (this._glideEnabled && this._jumpKeyHeld && this._inFreeFall) {
            this._freeFallDist = this._glideFallSpeed * dt;
        }
        else {
            // Increment the fall time by the delta time (dt).
            this._movFallTime = this._movFallTime + dt;
        }

        // Initialize variables to track movement status and action data.
        var moving = false;
        var actdata = null;

        // If the character is in free fall, adjust the movement vector accordingly.
        if (this._inFreeFall) {
            this._moveVector.y = -this._freeFallDist;
            moving = true;
        }

        // Rotate the avatar to align with the camera's direction.
        this._rotateAV2C();

        // Handle avatar and camera rotation based on player input.
        actdata = this._rotateAVnC(actdata, moving, dt);

        if(!this._checkGroundBelow()) {
            this._inFreeFall = true;

            //console.log('should be in free fall');
            //if(typeof this._moveVector == 'undefined') {
            //    this._moveVector = new BABYLON.Vector3(0,-5,0);
            //}
            this._act._stepLeft = false;
            this._act._stepRight = false;
            this._act._walk = false;
            this._act._walkback = false;
            this._moveVector = new BABYLON.Vector3(0,-1,0);
        ...
        // Check for conditions that would prevent the avatar from moving (e.g., not in free fall).
        if (!this._inFreeFall) {
            // Reset flags indicating walking or running status.
            this._wasWalking = false;
            this._wasRunning = false;

            // Variables to determine the direction and horizontal distance of movement.
            var sign = void 0;
            var horizDist = 0;

            // Handle different movement actions based on the current action state.
            // - Case handling for stepping left, including speed adjustments and action data updates.
            // - Similar case handling for stepping right, walking, walking backwards, and other actions.
            // - Each case calculates the movement vector based on the action and updates the moving status.
            switch (true) {
        ...
        // Handle movement of the avatar if the moving flag is set, which is the cae if any
        // of the this._act._* flags are set to true as detected in the above switch statement.
        if (moving) {
            // Check if the movement vector magnitude is significant.
            if (this._moveVector.length() > 0.001) {

                // Move the avatar with collision detection.
                this._avatar.moveWithCollisions(this._moveVector);

                // Additional handling for character movement, including slope analysis and free fall checks.
                // Check if the avatar's y-position is higher than its start position, indicating upward movement or climbing.
                if (this._avatar.position.y > this._avStartPos.y) {
                    // Calculate the displacement of the avatar from its start position.
                    var actDisp = this._avatar.position.subtract(this._avStartPos);

                    // Determine the slope of the ground the avatar is currently on.
                    var _slp = this._verticalSlope(actDisp);

                    // If the slope is steeper than the maximum slope limit (_sl2),
                    // it indicates the avatar may be trying to climb a steep incline or steps.
                    if (_slp >= this._sl2) {
                        // Check if a step offset is defined, indicating the presence of climbable steps.
                        if (this._stepOffset > 0) {
                            // Initialize the vertical movement total if this is the start of a climb.
                            if (this._vMoveTot == 0) {
                                this._vMovStartPos.copyFrom(this._avStartPos);
                            }

                            // Accumulate the total vertical movement.
                            this._vMoveTot = this._vMoveTot + (this._avatar.position.y - this._avStartPos.y);

                            // If the total vertical movement exceeds the step offset, reset the avatar to the start of the climb.
                            ...
                            // If there is no step offset, reset the avatar's position to its start position.
                            this._avatar.position.copyFrom(this._avStartPos);
                            this._endFreeFall();
                        }
                    }
                    else {
                        // If the slope is less than _sl2, reset the vertical movement total.
                        this._vMoveTot = 0;

                        // If the slope is steeper than the minimum slope limit (_sl1) but less than the max (_sl2),
                        // it indicates the avatar is on a steep slope but not steep enough to be considered climbing.
                        if (_slp > this._sl1) {
                            this._fallFrameCount = 0;
                            this._inFreeFall = false;
                        }
                        else {
                            // If the slope is less steep than the minimum slope limit, end the free fall state.
                            this._endFreeFall();
                        }
                    }
                }

                // Check if the avatar's y-position is lower than its start position, indicating downward movement.
                else if ((this._avatar.position.y) < this._avStartPos.y) {
                    var actDisp = this._avatar.position.subtract(this._avStartPos);

                    // If the avatar's displacement is not equal to its movement vector,
                    // it indicates the avatar is on a slope.
                    if (!(this._areVectorsEqual(actDisp, this._moveVector, 0.001))) {
                        // If the slope is gentle (less steep than _sl1), end the free fall state.
                        if (this._verticalSlope(actDisp) <= this._sl1) {
                            this._endFreeFall();
                        }
                        else {
                            // If the slope is steep (steeper than _sl1), continue the free fall state.
                            this._fallFrameCount = 0;
                            this._inFreeFall = false;
                        }
                    }
                    else {
                        // If the displacement equals the movement vector, the avatar is in free fall.
                        this._inFreeFall = true;
                        this._fallFrameCount++;

                        // If the avatar has been in free fall for more than the minimum frame count, switch to fall action.
        ...
        // Return the current action data, which dictates the next movement or animation state.
        return actdata;
    };
    ...
    CharacterController.prototype._onKeyDown = function (e) {
        ...
        switch (e.key.toLowerCase()) {
            case this._actionMap.idleJump.key:
                ...
                if (this._act._jump && this._jumpTime > 0 && this._jumpsUsed < this._maxJumps - 1) {
                    // Mid-air press during a jump: restart the arc from the
                    // current height (double jump).
                    this._jumpsUsed++;
                    this._jumpTime = 0;
                }
                else if (this._inFreeFall && this._jumpsUsed < this._maxJumps - 1) {
                    // Mid-air press after walking off a ledge: convert the
                    // free fall into a fresh jump.
                    this._jumpsUsed++;
                    this._inFreeFall = false;
                    this._movFallTime = 0;
                    this._fallFrameCount = 0;
    ...
    CharacterController.prototype._onKeyUp = function (e) {
        ...
        switch (e.key.toLowerCase()) {
            case this._actionMap.idleJump.key:
                this._jumpKeyHeld = false;   // releasing the key ends a glide
                break;
            case "shift":
