...
| `test-portal-door.js` | Portal doors (the interior-cell replacement): first entry prompts for the sub-level's name (test-injected) and swaps the WHOLE scene to a seeded starter room with its own exit portal, cancelling the prompt cancels entry, edits inside persist across exit/re-entry, exiting restores the parent exactly (door keeps its name, the sub-level nests under the parent's `subWorlds`), saving from INSIDE stores the folded root world, dying inside respawns inside, and the door's `entered` wiring steps a quest whose progress survives the round trip. |
