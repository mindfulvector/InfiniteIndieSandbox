...
| `test-hub.js` | The Sandbox Hub starter world (New Game option 5): terrain + all four zones' objects exist with their pre-authored wires (trigger→spawner+camera, stars→counter→scoreboard, path chain→ferry, patrol), and the wires PLAY — walking into the Combat Yard spawns walkers and cuts the camera, the ferry moves, the guard patrols, and touching a star increments the climb counter. |
| `test-interior-cells.js` | The pocket-interior door: walking in teleports the player to a far-away decorated room (raw meshes, ≥8 built), outdoor enemies freeze via insideCell, `entered`/`exited` fire wired counters, the exit pad returns the player placed clear of the trigger radius (no yo-yo even past the cooldown), dying inside respawns outside with the state cleared, build-mode return disposes every cell mesh, and the cell never serializes. |
...
| `test-coop.js` | The drop-in buddy (local 2P v1): B toggles a bipedal rig beside P1, the injected second-pad stick moves it (camera-relative + walk cycle), a held jump launches it from the ground, its pad attack lands the frontal-arc melee from the buddy's own position, falling off the world rescues it to P1's side, a second pad's join flag drops it in keyboard-free, and leaving disposes the rig. |
