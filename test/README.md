...
| `test-paths.js` | Paths + moving platform: a 3-node wire chain resolves, the platform snaps to node 1 on play start and travels dt-based, `arrived` fires a counter at each node, `once` stops at the end firing `completed` exactly once, `stop`/`start` freeze/resume, `pingpong` turns around, respawn returns it to the start, and restPos keeps a mid-route save at the build-time home. |
