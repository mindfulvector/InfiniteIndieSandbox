...
| `test-spitter.js` | The en_spitter artillery enemy: registers isEnemy + lobbed/impact outputs, lobs a shot in range that arcs above the straight line, a shot landing on the player splash-damages them, moving away before it lands dodges it (no cover), and melee defeats it and clears its in-flight shots. |
...
| `test-stampline.js` | Build-mode row stamp (L): stamping lays 4 copies marching along +X evenly stepped by the object's footprint, the copies carry the original's rotation/scale, all register on the undo stack, the row (original + copies) becomes the selection, and stamping with nothing selected is a harmless no-op. |
...
| `test-streak-hud.js` | The defeat-streak HUD readout: a hudStreak element hidden with no chain, hidden at streak 1 and shown at streak 2, its text carries the streak count and the live multiplier from streak 3, its colour heats up by tier (blue→gold→orange→red), and resetting the streak hides it again. |
| `test-streak.js` | The defeat-streak pixel multiplier: streakMult tiers rise 1→1.5→2→3, chaining defeats via defeatEnemy raises the streak, a multiplied defeat drops bonus pixels, a landed hit breaks the streak while a dodged/blocked hit preserves it, and death resets it. |
...
| `test-plate.js` | The l_plate pressure plate: registers with PlateScript + pressed/released outputs, standing on it fires pressed once and sinks it (opening a wired door), stepping off fires released and rises it (closing the door, momentary), a latch plate stays pressed after you step off, and a play reset lifts it back up. |
| `test-spinner.js` | The t_spinner rotating platform: registers with SpinnerScript + a spun output and on/off inputs, the disc rotates over time, a rider is carried around the pivot (offset rotates, radius preserved — an orbit not a drift), a rider near the rim swings a wider arc than one near the hub, the direction param reverses the orbit, and a rider off the disc isn't carried. |
...
| `test-text-entry.js` | The reusable in-game text field behind `app.promptText`: opens with label + prefilled default, freezes the world while open (enemy frames stop) and unfreezes on close, confirm returns trimmed text while typed keys never leak into game actions, cancel/empty-confirm return null, only one modal opens at a time, and the real save-world call site works end to end through the field. |
| `test-portal-door.js` | Portal doors (the interior-cell replacement): first entry prompts for the sub-level's name (test-injected) and swaps the WHOLE scene to a seeded starter room with its own exit portal, cancelling the prompt cancels entry, the player arrives (and returns) facing away from the door, edits inside persist across exit/re-entry, exiting restores the parent exactly (door keeps its name, the sub-level nests under the parent's `subWorlds`), saving from INSIDE stores the folded root world, dying inside respawns inside, the door's `entered` wiring steps a quest whose progress survives the round trip, and the texture/tint look settings re-skin both door types (glow panel takes the tint). |
...
| `test-campaign.js` | The Glowlands Play Set, played end to end: the 3-mission chain imports wired, the vault door blocks before M1, collecting the stars pays M1 and slides the vault open, stepping through the portal door into its sub-level pays M2 (quest state surviving the world swap via mirrored params), a zero-checkpoint lap at the gate pays M3 with the scoreboard and camera finale — proving campaigns package from shipped toys with no engine additions. |
...
| `test-shellfall-world.js` | The Shellfall bombardment world end to end: imports with a checkpoint, turret, spitter, cover pillars, a wave spawner (wired to the vault), and a regen field; the in-world spitter lobs a shot that arcs over the 2-tall cover pillars; the regen field heals a hurt player; clearing the wave opens the vault and steps the quest; and the vault chest pays out. |
| `test-locks-world.js` | The Warden's Locks plate-puzzle world end to end: imports with a checkpoint, three latch plates, two doors, a counter (target 2), and the vault; weighing down plate P1 opens door D1 and steps the quest; one plate alone does not open D2 but pressing both trips the counter AND-gate and opens it; and the vault chest pays out. |
